...
def fetch_from_config(ds_config: dict, base_dir: str | Path = "data/raw") -> dict:
    """
    Pipeline-compatible entry point. Called by run_all with a dataset config dict.
    
    Parameters
    ----------
    ds_config : dict
        Dataset config from datasets.yaml with fields: mode, organism, tissue_general,
        disease, primary_only, assay, max_cells, random_state, measurement_name,
    ...
    Returns
    -------
    dict with keys: id, status, output_path, cells, error
    """
    import logging
    setup_logging()
    
    base_dir = Path(base_dir) / "cellxgene"
    base_dir.mkdir(parents=True, exist_ok=True)
    
    ds_id = ds_config.get("id", "unknown")
    mode = ds_config.get("mode", "census_slice")
    output_filename = ds_config.get("output_filename", f"{ds_id}.h5ad")
    output_path = base_dir / output_filename
    
    result = {"id": ds_id, "status": "pending", "output_path": str(output_path), "cells": 0, "error": None}
    
    try:
        if mode == "census_slice":
            census_slice_to_h5ad(
                output=output_path,
                organism=ds_config.get("organism", "Homo sapiens"),
                tissue_general=ds_config.get("tissue_general", "heart"),
                disease=ds_config.get("disease"),
                primary_only=ds_config.get("primary_only", True),
                assay=ds_config.get("assay"),
                max_cells=ds_config.get("max_cells", 50000),
                random_state=ds_config.get("random_state", 42),
                measurement_name=ds_config.get("measurement_name", "RNA"),
            )
            if output_path.exists():
                import anndata
                adata = anndata.read_h5ad(output_path, backed="r")
                result["cells"] = adata.n_obs
                adata.file.close()
            result["status"] = "success"
            
        elif mode == "source_h5ad":
            dataset_id = ds_config.get("dataset_id")
            if not dataset_id:
                raise ValueError("dataset_id required for source_h5ad mode")
            download_source_h5ad(dataset_id, output_path)
            result["status"] = "success"
            
        elif mode == "source_uri":
            dataset_id = ds_config.get("dataset_id")
            if not dataset_id:
                raise ValueError("dataset_id required for source_uri mode")
            print_source_uri(dataset_id)
            result["status"] = "success"
        else:
            raise ValueError(f"Unknown mode: {mode}")
            
    except Exception as e:
        logging.error("CELLxGENE fetch failed for %s: %s", ds_id, e)
